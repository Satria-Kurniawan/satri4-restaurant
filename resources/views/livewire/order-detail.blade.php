...
<div class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen  text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-top bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-top sm:max-w-lg sm:w-full">
                    ...
                    <table class="">
                        ...
                        @forelse ($carts as $cart)
                            <tbody>
                                <tr>
                                    <td>{{ $cart['name'] }}</td>
                                    <td>{{ number_format($cart['pricesingle'],2,',','.') }}</td>
                                    <td>{{ $cart['qty'] }}</td>
            ...
            <p class="font-bold ml-4 mb-0 mt-2">Total Harga : Rp. {{ number_format($summary['total'],2,',','.') }}</p>
            <p class="font-bold ml-4">Atas nama : {{{ Auth::user()->name}}}</p>
        <div class="p-3">
            <form wire:submit.prevent="handleSubmit">
                <div class="mt-4 mb-2">
                    <button wire:ignore type="submit" id="saveButton" class="btn btn-success btn-block">
                        Conrifm<i class="ml-2 fas fa-check"></i>
                    </button>
                </div>
            </form>
            <button wire:click = "hideModal()" type="button" class="btn btn-danger btn-block">
                Cancel<i class="ml-2 fas fa-times"></i>
            </button>
        </div>
      </div>
    </div>
